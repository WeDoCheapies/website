...
interface EditCarDialogProps {
  ...
  onClose: () => void;
  car: Car;
  onSubmit: (updatedCar: Partial<Car>) => void;
}

const EditCarDialog = ({ isOpen, onClose, car, onSubmit }: EditCarDialogProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFiles.length > 0) {
        newImageUrls = await carService.uploadCarImages(imageFiles);
      }
      ...
      const updatedCar: Partial<Car> = {
        ...formData,
        year: typeof formData.year === 'string' ? parseInt(formData.year as string) : formData.year,
      ...
      // Handle case for removed images
      const removedImages = (car.image_urls || [])
        .filter(url => !existingImages.includes(url));
      
      if (removedImages.length > 0) {
        // Try to delete removed images (non-blocking)
        try {
          await carService.deleteCarImages(removedImages);
        } catch (error) {
      ...
      <DialogContent className="sm:max-w-xl md:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Add New Images */}
            <div className="md:col-span-2">
              <Label htmlFor="car-images">Add New Images</Label>
              <Input
                id="car-images"
                type="file"
                accept="image/*"
              ...
              <p className="text-xs text-gray-500 mt-1">
                You can select multiple new images to add to this car.
              </p>
                ...
                <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                  {imagePreviews.map((preview, index) => (
                    <div key={`new-${index}`} className="relative group">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-blue-500 bg-opacity-50 text-white text-xs py-1 px-2">
                        New Image
                      </div>
                    </div>
                  ))}
